   ...
   public function notifUrlAction(Request $request)
    ...
    public function notifVideoAction(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($video_selected->getVideo()->getEnabled()==true) {
                $videourl = str_replace("/media/cache/resolve/image/", "/", $imagineCacheManager->getBrowserPath($video_selected->getVideo()->getLink(), 'image'));

            }else{
                $videourl = $video_selected->getVideo()->getLink();
            }
            ...
            $message = array(
                  ...
                  "video_description"=> $video_selected->getDescription(),
