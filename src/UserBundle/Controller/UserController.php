...
class UserController extends Controller
{
   ...
   public function api_getAction(Request $request,$user,$me,$token){
    ...
    public function statusAction(Request $request,$id)
    {
        ...
        $dql        = "SELECT w FROM AppBundle:Video w  WHERE w.user = ".$user->getId() ." ORDER BY w.created desc";
        $query      = $em->createQuery($dql);
        $paginator  = $this->get('knp_paginator');
        $pagination = $paginator->paginate(
            $query,
    ...
    public function  api_followingstopAction(Request $request,$user,$token)
    {
        ...
        foreach ($user->getUsers() as $key => $e) {
            if (sizeof($e->getStatus())>0) {
               ...
               $b["name"] = $e->getName();
               $b["image"] = $e->getImage();
               $last_wallpaper = $em->getRepository('AppBundle:Video')->findOneBy(array("enabled"=>true,"user"=>$e),array("created"=>"desc"));
